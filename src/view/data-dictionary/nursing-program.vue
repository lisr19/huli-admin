<template>
  <div>
    ...
    <Modal v-model="isDetail" :mask-closable="false" :closable="false" :title="'('+serviceName+')子服务：'" width="1000">
      <nursing-program-detail ref="nursingDetail" ></nursing-program-detail>
      <!--自定义页脚-->
      <div slot="footer">
        <Button type="text" @click="isDetail = false">返回</Button>
      </div>
    </Modal>

    <!--服务耗材-->
    <Modal  v-model="isConsume" :mask-closable="false" :closable="false" :title="'('+serviceName+')耗材详情：'" width="1000">
      <program-consume ref="serviceConsume"></program-consume>
      <div slot="footer">
        <Button type="text" @click="isConsume = false">返回</Button>
      </div>
    </Modal>
  </div>
</template>
...
<script>
  ...
  import nursingProgramDetail from './nursing-program-detail'
  import programConsume from './program-consume'

  export default {
    name: "nursing-program",
    components: {
      ImgUpload,
      nursingProgramDetail,
      programConsume
    },
    computed: {
      access() {
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            key: 'handle',
            render: (h, params) => {
              if (this.viewChild || this.viewConsume ||this.viewEdit || this.viewDel) {
                return h('div', [
                  h('Button', {
                    props: {
                      type: 'success'
                  ...
                  h('Button', {
                    ...
                    style: {
                      marginRight: '5px',
                      display: this.viewConsume? 'inline-block' : 'none'
                    },
                    on: {
                      click: () => {
                        this.isConsume = true
                        this.$refs.serviceConsume.serviceId = params.row.id
                        this.serviceName = params.row.name
                        this.$refs.serviceConsume.findServiceConsume({serviceId:this.serviceId})
                      }
                    }
                  }, '耗材'),
                  h('Button', {
