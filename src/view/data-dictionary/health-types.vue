  ...
  <div>
      ...
      <div>
        ...
        >
          <template slot="action" slot-scope="{row}">
            ...
            <Button type="error" style="width: 80px;" v-if="viewDel" @click="doTypesDel(row)">删除</Button>
          </template>
        </tree-table>
      </div>
    </Card>
    ...
    <Modal v-model="isAdd" :mask-closable="false" :closable="false" title="类型添加：" width="500">
      <Form :model="addForm" ref="addForm" :label-width="100" :rules="rules">
        ...
        <FormItem label="父级类型：">
          <Cascader :data="typesCas" v-model="addForm.typesCas" placeholder="角色的父级类型"></Cascader>
        </FormItem>
      </Form>
      <!--自定义页脚-->
      <div slot="footer">
    ...
    <Modal v-model="isEdit" :mask-closable="false" :closable="false" title="类型编辑：" width="500">
      <Form :model="editForm" ref="editForm" :label-width="100" :rules="rules">
        ...
        <FormItem label="父级类型：">
          <Cascader :data="typesCas" v-model="editForm.typesCas" placeholder="角色的父级类型"></Cascader>
        </FormItem>
      </Form>
      <!--自定义页脚-->
      <div slot="footer">
...
<script>
  import {healthTypesColumns} from "../../libs/table";
  ...
  export default {
    ...
    created(){
      this.columns = healthTypesColumns
    },
    methods:{
      //类型删除
      doTypesDel(row){
